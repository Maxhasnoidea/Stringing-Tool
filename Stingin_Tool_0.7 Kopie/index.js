...
function createTubeMeshFromConnections() {
    if (connections.length === 0) {
        console.warn('No connections available to create a mesh');
        return;
    }
    ...
    connections.forEach(connection => {
        ...
        let points;
        if (curve) {
            // If the connection is a Bezier curve, get the points from the curve
            points = curve.getPoints(50);
        } else {
            // If the connection is a straight line, create points from start to end
            points = [start, end];
        }

        // Add points to pathPoints array
        points.forEach(point => {
            pathPoints.push(new THREE.Vector3(point.x, point.y, point.z));
        });
    });

    // Create a CatmullRomCurve3 from the path points
    const curvePath = new THREE.CatmullRomCurve3(pathPoints);

    // Create the tube geometry
    const tubeGeometry = new THREE.TubeGeometry(curvePath, 100, meshThickness, 8, false);

    // Create the material for the tube mesh with purple color
    const tubeMaterial = new THREE.MeshPhongMaterial({ color: 0x800080 });

    // Remove the old tube mesh if it exists
    if (tubeMesh) {
